...
{
	target_parameter_set(120, 90, 60, 60, 60);		//相对位移90°，6°/s，电机加速度减速度均为50.
	task_start();
	isMotorTaskFinished();
}	
...
void Move2Zero(void)
{
	target_parameter_set(120, 0, 60, 60, 60);		//相对位移90°，6°/s，电机加速度减速度均为50.
	task_start();
	isMotorTaskFinished();
}
...
void Action_Drink(void)
{
	...
	while(num < 5)
	{
		...
		{
			flag = 0;
			target_parameter_set(120, 108, 30, 60, 60);
			task_start();
			isMotorTaskFinished();
			task_time_set(20);
		}
		...
		{
			flag = 0;
			target_parameter_set(120, 126, 45, 60, 60);
			task_start();
			isMotorTaskFinished();
			task_time_set(20);
		}
		...
		{
			flag = 0;
			target_parameter_set(120, 108, 45, 60, 60);
			task_start();
			isMotorTaskFinished();
			task_time_set(20);
		}
