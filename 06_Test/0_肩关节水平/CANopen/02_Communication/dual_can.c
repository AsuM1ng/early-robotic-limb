...
{	
  ...
  CAN_Init(CAN2, &CAN_InitStructure);
	...
	CAN_FilterInitStructure.CAN_FilterMode=CAN_FilterMode_IdMask;
	CAN_FilterInitStructure.CAN_FilterScale=CAN_FilterScale_32bit;
	CAN_FilterInitStructure.CAN_FilterIdHigh=0x666<<5;
	CAN_FilterInitStructure.CAN_FilterIdLow=0x0000;
	
	CAN_FilterInitStructure.CAN_FilterMaskIdHigh=0x7FFF;
	CAN_FilterInitStructure.CAN_FilterMaskIdLow=0x0000;
	CAN_FilterInitStructure.CAN_FilterFIFOAssignment=CAN_Filter_FIFO0;
	CAN_FilterInitStructure.CAN_FilterActivation=ENABLE;
	CAN_FilterInit(&CAN_FilterInitStructure);
...
void CAN1_RX0_IRQHandler(void)
{
  ...
  if(Rx1Message.RTR == CAN_RTR_REMOTE)//远程帧
	...
	index = rxm1.data[3] + (rxm1.data[1]<<8) + (rxm1.data[2]<<16);
	receive_data = rxm1.data[4] + (rxm1.data[5] << 8) + (rxm1.data[6] << 16) + (rxm1.data[7] << 24);
/****  接收来自EPOS的反馈信息  ****/

		if(rxm1.cob_id == 0x581 && (rxm1.data[0] == 0x43 || rxm1.data[0] == 0x4B))
		{
			switch(index)
			{
				//电机参数读取
				case 0x606400: MD.Motor_Position = receive_data; break;
				case 0x608100: MD.Profile_Velocity_Target = receive_data; break;
				case 0x608300: MD.Profile_Acceleration = receive_data; break;
				case 0x608400: MD.Profile_Deceleration = receive_data; break;
				
				//电机状态读取
				case 0x604000: MS.Controlword = rxm1.data[4] + (rxm1.data[5] << 8); break;
				case 0x604100: MS.Statusword = rxm1.data[4] + (rxm1.data[5] << 8); break;
				case 0x220001: MD.Power_Supply = (u8)receive_data / 10; break;
				case 0x100301: MS.Error_Code = receive_data; break;
		...
		if(TS.waiting)
		{
			switch(rxm2.data[3])
			{
				case 0x01:
					...
					TC.anyother = 0;
					if(TC.ready){PDout(2) = !PDout(2);PAout(8) = !PAout(8);}
				break;
				...
				case 0x02:
					...
					TC.Stretching_exercise = 0;
					TC.anyother = 0;
					if(TC.halt){PDout(2) = !PDout(2);PAout(8) = !PAout(8);}
				break;
				...
				case 0x03:
					...
					TC.Stretching_exercise = 0;
					TC.anyother = 0;
					if(TC.zero){PDout(2) = !PDout(2);PAout(8) = !PAout(8);}
				break;
				...
				case 0x04:
					...
					TC.Stretching_exercise = 0;
					TC.anyother = 0;
					if(TC.drink){PDout(2) = !PDout(2);PAout(8) = !PAout(8);}
				break;
				...
				case 0x05:
					...
					TC.zero = 0;
					TC.drink = 0;
					TC.Stretching_exercise = 1;
					TC.anyother = 0;
					if(TC.Stretching_exercise){PDout(2) = !PDout(2);PAout(8) = !PAout(8);}
				break;
				...
				case 0x06:
					...
					TC.Stretching_exercise = 0;
					TC.anyother = 1;
					if(TC.anyother){PDout(2) = !PDout(2);PAout(8) = !PAout(8);}
				break;
