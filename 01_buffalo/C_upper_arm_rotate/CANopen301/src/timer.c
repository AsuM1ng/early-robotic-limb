...
// IRQ: CANopen_Master_node
void TIM_CANopenMasterIRQHandler(void)
{
	if (TIM_GetITStatus(TIM_CANOPEN_MASTER, TIM_IT_Update) != RESET)
	{
		CANopenMaster_Data.last_time_set = TIM_GetCounter(TIM_CANOPEN_MASTER);

		TimeDispatch(&CANopenMaster_Data);
        
    TIM_ClearITPendingBit(TIM_CANOPEN_MASTER, TIM_IT_Update);
	}
}
