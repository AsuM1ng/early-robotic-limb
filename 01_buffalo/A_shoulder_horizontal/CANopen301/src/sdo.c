...
*//* 上传或下载的启动阶段因为只有data[4]~data[7]可能会包含数据，且当n有效时，n的可能取值只能是1,2,3这三个数，意思就是当n有效时间n最大是3，所有这个时候n只需要两位 */
...
void SDOTimeoutAlarm(CO_Data* d, UNS32 id)
{
    ...
    {
        while(d->Find_Object(NULL, 0x12800000|offset<<16, &errorCode, d)!=&NULL_OBJ)
        {offset = offset + 1;}
    }
    if((d->transfers[id].whoami) == SDO_SERVER)
    {
        while(d->Find_Object(NULL, 0x12000000|offset<<16, &errorCode, d)!=&NULL_OBJ)
        {offset = offset + 1;}
    }

    if ((d->transfers[id].CliServNbr)+1 > offset)
	{
		return ;
	}
    ...
    if((d->transfers[id].whoami) == SDO_CLIENT)
    {
         nodeId = (UNS8) *((UNS32*) (d->Find_Object( NULL, 0x12800003 | (d->transfers[id].CliServNbr<<16), &errorCode, d)) -> p_content);
    }
    if((d->transfers[id].whoami) == SDO_SERVER)
    {
         nodeId = (UNS8) *((UNS32*) (d->Find_Object( NULL, 0x12000003 | (d->transfers[id].CliServNbr<<16), &errorCode, d)) -> p_content);
    }
	...
	if( ((d->transfers[id].whoami) == SDO_CLIENT) && (d->transfers[id].Callback) )
    {
...
//	if( ((d->transfers[id].whoami) == SDO_SERVER) || (d->transfers[id].abortCode == SDOABT_TIMED_OUT) )
//    {
 ...
 ** @param line
 ** @param nbBytes
 ** @param data   用于获取从line中拷出的数据
 **
 ** @return
 **//* 用于分段传输时，将line中的数据一次一次的挪到* data中用SDO数据帧中发送出去，一般每次从line中挪出7个字节到SDO帧中 */
...
#else //SDO_DYNAMIC_BUFFER_ALLOCATION
	for (i = 0 ; i < nbBytes ; i++)
	* (data + i) = d->transfers[line].data[offset + i];/* 从该线的d->transfers[line].data的第d->transfers[line].offset个字节上开始拷贝出nbBytes个字节的数据 */
#endif //SDO_DYNAMIC_BUFFER_ALLOCATION
	d->transfers[line].offset = d->transfers[line].offset + nbBytes;/* 考出多少字节数据，offset往后移多少位，用于指明offset之前的数据字节已经转移出去了，下一次从offset处开始拷出数据就行了 */
	return 0;
}
...
UNS8 failedSDO (CO_Data* d, UNS8 CliServNbr, UNS8 whoami, UNS16 index, UNS8 subIndex, UNS32 abortCode)
{
    ...
    {
         nodeId = (UNS8) *((UNS32*) (d->Find_Object( NULL, 0x12800003 | (CliServNbr<<16), &errorCode, d)) -> p_content);
    }
    if(whoami == SDO_SERVER)
    {
         nodeId = (UNS8) *((UNS32*) (d->Find_Object( NULL, 0x12000003 | (CliServNbr<<16), &errorCode, d)) -> p_content);
    }
    
    /* Call the user function to inform of the problem.*/
	if( ((d->transfers[line].whoami) == SDO_CLIENT) && (d->transfers[line].Callback) )
    {
        /*If ther is a callback, it is responsible to close SDO transfer (client)*/
//		(*d->transfers[line].Callback)(d, nodeId);
    }
    
	/*Reset the line if the callback did not close the line.
	  Otherwise this sdo transfer would never be closed. */
	if( d->transfers[line].abortCode == abortCode)
    {
//        resetSDOline(d, (UNS8)line);
    }
...
void resetSDOline ( CO_Data* d, UNS8 line )
{
    ...
    d->transfers[line].shadow_of_state = 0;
...
//		m.cob_id = (UNS16) *((UNS32*) d->objdict[offset+CliServNbr].pSubindex[2].pObject);/* 服务器->客户端COB_ID */
		m.cob_id = (UNS16) *((UNS32*) d->Find_Object(NULL, 0x12000002|CliServNbr<<16, &errorCode, d)->p_content);/* 服务器->客户端COB_ID */
...
//		m.cob_id = (UNS16) *((UNS32*) d->objdict[offset+CliServNbr].pSubindex[1].pObject);/* 客户端->服务器COB_ID */
		m.cob_id = (UNS16) *((UNS32*) d->Find_Object(NULL, 0x12800001|CliServNbr<<16, &errorCode, d)->p_content);/* 服务器->客户端COB_ID */
...
//int test_download_start_request = 0;
//int test_segmeng_download_request = 0;
...
//	lastIndex = d->lastIndex->SDO_SVR;
    ...
    {
        SdoReadAnalysis(&Sdo_R_Para, m);
//        return 0x00;
    }
    
    if(WriteProcess == Sdo_W_Para.Sdo_Status)
    {
        SdoWriteAck(&Sdo_W_Para, m);
//        return 0x00;
    }
...
//			if (d->objdict[offset].bSubCount <= 1)
		    if (*((UNS8*)d->Find_Object(NULL, 0x12000000|num<<16, &ErrorCode, d)->p_content) < 1)
			{
...
//			pCobId = (UNS32*) d->objdict[offset].pSubindex[1].pObject;/* 客户端->服务器COB-ID，这里取出它是为了进行比较，因为这里既然要判断它是不是SERVER，那就要判断它SDO服务器参数对象中的客户端->服务器COB-ID是否与接收到的帧m的COB-ID相等  */
			pCobId = (UNS32*) d->Find_Object(NULL, 0x12000001|num<<16, &ErrorCode, d)->p_content;
			if ( *pCobId == UNS16_LE(m->cob_id) )
			{
				...
				CliServNbr = j;               
                nodeId = *((UNS8*) d->Find_Object(NULL, 0x12000003|num<<16, &ErrorCode, d)->p_content);
				break;
			}
			j++;
//			offset++;
...
//				if (d->objdict[offset].bSubCount <= 3)
				if (*(UNS8*)d->Find_Object(NULL, 0x12800000|num<<16, &ErrorCode, d)->p_content < 3)
				{
...
//				pCobId = (UNS32*) d->objdict[offset].pSubindex[2].pObject;/* 服务器->客户端COB-ID  */
				pCobId = (UNS32*) d->Find_Object(NULL, 0x12800002|num<<16, &ErrorCode, d)->p_content;
				if (*pCobId == UNS16_LE(m->cob_id) )
				{
					whoami = SDO_CLIENT;
...
//					nodeId = *((UNS8*) d->objdict[offset].pSubindex[3].pObject);
					nodeId = *((UNS8*) d->Find_Object(NULL, 0x12800003|num<<16, &ErrorCode, d)->p_content);
					break;
				}
				j++;
//				offset++;
	...
	switch (cs) 
	{
		case 0:
			/* I am SERVER */
		if (whoami == SDO_SERVER)/* 服务器接收到客户端发来的分段下载请求，见SDO分段下载协议301page45*/
			{
                ...
                if(index == 0x60c1)
                {
                    d->test_segmeng_download_request++;
                }
			...
			{ /* if CLIENT *//* 客户端接收到服务器的分段上传响应        见SDO分段上传协议301page48*/
				/* I am CLIENT */
		...
		if (whoami == SDO_SERVER)
			...
			{/* 客户端接收到服务器的分段下载响应，见SDO分段下载协议301page45 */
				/* I am CLIENT */
		...
		case 2:
			...
			{/* 客户端接收到服务器的上传启动响应，见SDO上传启动协议301page47 */
				/* I am CLIENT */
		...
		case 3:
			...
			{/* 客户端接收到服务器的下载启动响应，见SDO下载启动协议301page44 */
				/* I am CLIENT */
...
//                         resetSDOline(d, (UNS8)line);
		...
		case 5: /* Command specifier for data transmission - the client or server is the data producer */
			...
			if (whoami == SDO_SERVER) /* Server block upload */
			{
				...
				else if ((SubCommand == SDO_BCS_UPLOAD_RESPONSE) || (SubCommand == SDO_BCS_START_UPLOAD)) 
				{
					...
					index = d->transfers[line].index;
					subIndex = d->transfers[line].subIndex;
					if (SubCommand == SDO_BCS_UPLOAD_RESPONSE)/* 收到客户端的子块结束相应 */
					{
						...
						{ /* Si tout est envoyé et confirmé reçu on envoi un block end upload response */
					    ...
					    /* (SubCommand == SDO_BCS_START_UPLOAD)的情形到此结束 */
					
					/* 上传子块，有两种情况，一种是上传启动协议之后，一种是还有子块要传输 */
					/* (SubCommand == SDO_BCS_UPLOAD_RESPONSE) || (SubCommand == SDO_BCS_START_UPLOAD)都要执行下面部分 */
			...
			{ /* if CLIENT (block download) */
                if ((SubCommand == SDO_BSS_INITIATE_DOWNLOAD_RESPONSE) || (SubCommand == SDO_BSS_DOWNLOAD_RESPONSE)) 
				{
                    ...
                    else
					{
                        ...
                        getSDOlineRestBytes(d, line, &nbBytes);
                        if((nbBytes == 0) && (AckSeq == d->transfers[line].seqno))
						{ /* Si tout est envoyé et confirmé reçu on envoi un block end download request */
		...
		case 6: /* Command specifier for data reception - the client or server is the data consumer */
			if (whoami == SDO_SERVER) 
			{ /* 服务器接收到客户端的块下载请求 *//* Server block download */
				if (err) 
				{/* 没找到，需要需要新开一个传输通道 */
...
//		nodeIdServer = *((UNS8*) d->objdict[offset].pSubindex[3].pObject);
		nodeIdServer = *((UNS8*) d->Find_Object(NULL, 0x12800003|num<<16, &ErrorCode, d)->p_content);
...
//		offset++;
	...
	if (!SDOfound)
	{
		...
		return 0xFE;/* 虽然定义过CSDO，但是他们的服务器节点ID均不是传进来的函数参数nodeId（CSDO的子索引0x03） */
	}
...
//int writeNetworkDictlost1=0;
//int writeNetworkDictlost2=0;
//int writeNetworkDictlost3=0;
//int writeNetworkDictlost4=0;
/*!
 **
 **
 ** @param d
...
**//* 主站想要向别的节点请求数据，其实本质上调用的就是该函数，虽然有时候不是直接调用 */
/* SDO块下载或下载启动协议，要不要用块下载方式由用户自己设置函数参数 useBlockMode 来决定
当不用块传输的时候，数据量 count 不超过4字节会自动使用加速SDO的方式，否则的话会使用一般的SDO传输方式*/
							   ...
							   void *data, SDOCallback_t Callback, UNS8 endianize, UNS8 useBlockMode)
{
	...
	if(CliNbr >= 0xFE)/* 0xFF是没定义CSDO或定义了但强制的子索引3没有实现；0xFE是定义了CSDO但是他们的服务器节点ID均不是nodeId；0x00才是找到成功 */
	{
		//writeNetworkDictlost1++;		
		return CliNbr;
	}
	/* Verify that there is no SDO communication yet. */
	err = getSDOlineOnUse(d, CliNbr, SDO_CLIENT, &line);
	if (!err) {/* 判断这个传输有没有已经存在并正在进行中 */
		MSG_ERR(0x1AC4, "SDO error : Communication yet established. with node : ", nodeId);
		//writeNetworkDictlost2++;
		return 0xFD;/* 找到，说明这个传输正在进行中，就让它继续进行，不用再管它 */		
		//return 0xFF;/* 找到，说明这个传输正在进行中，就让它继续进行，不用再管它 */
	}
	/* 能运行到这里说明该传输没有在进行中，说明是一个新的传输，下面就需要为它新开一个传输通道 */
	/* Taking the line ... */
	err = getSDOfreeLine( d, SDO_CLIENT, &line );
	if (err) {/* 判断有没有空闲的通道供这个传输使用 */
		MSG_ERR(0x1AC5, "SDO error : No line free, too many SDO in progress. Aborted for node : ", nodeId);
		//writeNetworkDictlost3++;
		return (0xFF);/* 所有通道都被占用，说明允许的传输通道定义的不够大，建议改大一点以支持更多的同时传输 */
	}
	else
    ...
    if(useBlockMode) {
        ...
        d->transfers[line].shadow_of_state = SDO_BLOCK_DOWNLOAD_IN_PROGRESS;
	    d->transfers[line].objsize = count;
    }
    else {
	    initSDOline(d, line, CliNbr, index, subIndex, SDO_DOWNLOAD_IN_PROGRESS);
        d->transfers[line].shadow_of_state = SDO_DOWNLOAD_IN_PROGRESS;
    }
...
#  endif
    ...
    if(useBlockMode)/* 客户端块下载启动请求，见SDO块下载启动协议，301page50 */
	{
	    buf[0] = (6 << 5) | (1 << 1 );   /* CCS = 6 , CC = 0 , S = 1 , CS = 0 */
 	    for (i = 0 ; i < 4 ; i++)
		    buf[i+4] = (UNS8)((count >> (i<<3))); /* i*8 */
    }
    else /* SDO下载启动协议301page44 */
	{
	    /* Send the SDO to the server. Initiate download, cs=1. */
	    if (count <= 4) { /* Expedited transfer *//* 加速SDO，后面四个字节可以直接把数据传过去 */
		    buf[0] = (UNS8)((1 << 5) | ((4 - count) << 2) | 3);
		    for (i = 4 ; i < 8 ; i++)/* 这样搞会不管count为多少都要拷贝4个字节到buf中，为什么不用 i < 4+count 来限制*/
			buf[i] = d->transfers[line].data[i - 4];/* 可能是多拷贝进去的不一定会有用，别人读的时候会根据n(n是什么意思见301page44)来取，多的不要就行了 */
...
//				*(UNS32*)d->objdict[offset].pSubindex[1].pObject = (UNS32)(0x600 + nodeId);
				*((UNS32*) d->Find_Object(NULL, 0x12800001|num<<16, &ErrorCode, d)->p_content) = (UNS32)(0x600 + nodeId);
//				*(UNS32*)d->objdict[offset].pSubindex[2].pObject = (UNS32)(0x580 + nodeId);
				*((UNS32*) d->Find_Object(NULL, 0x12800002|num<<16, &ErrorCode, d)->p_content) = (UNS32)(0x580 + nodeId);
//				*(UNS8*) d->objdict[offset].pSubindex[3].pObject = nodeId;
				*((UNS8*) d->Find_Object(NULL, 0x12800003|num<<16, &ErrorCode, d)->p_content) = nodeId;
...
//int readNetworkDictlost1=0;
//int readNetworkDictlost2=0;
//int readNetworkDictlost3=0;
//int readNetworkDictlost4=0;
/*!
 **
 **
 ** @param d
							...
							UNS8 dataType, SDOCallback_t Callback, UNS8 useBlockMode)
{
    ...
    if(useBlockMode) {/* 块上传启动请求见301Page54 */
        ...
        d->transfers[line].shadow_of_state = SDO_BLOCK_UPLOAD_IN_PROGRESS;
        ...
        d->transfers[line].shadow_of_state = SDO_UPLOAD_IN_PROGRESS;
...
//	UNS8 i;
	...
	if(ret == 0xFE)  //定义过CSDO，但nodeId没匹配上
	{
//		offset = d->firstIndex->SDO_CLT;
//		lastIndex = d->lastIndex->SDO_CLT;
		while(d->Find_Object(NULL, 0x12800000|offset<<16, &ErrorCode, d)!=&NULL_OBJ)
 ...
 **/
//int getReadResultNetworkDict_num = 0;
//int line_error = 0;
UNS8 getReadResultNetworkDict(CO_Data* d, UNS8 *nowline, UNS8 nodeId, void* data, UNS32 *size, UNS32 * abortCode)
{
	UNS32 i;
	UNS8 err;
	UNS8 CliNbr;
 ...
 ** @return
 **/
//int getWriteResultNetworkDict_error = 0;
//int SDO_FINISHED_NUM = 0;
//int resetSDOline_num = 0;
UNS8 getWriteResultNetworkDict (CO_Data* d, UNS8 *nowline, UNS8 nodeId, UNS32 * abortCode)
{
	UNS8 line = 0;
	UNS8 err;
	UNS8 CliNbr;
...
//    if(line != 0)
//    {
//        getWriteResultNetworkDict_error++;
//    }	
	if (err) 
	{
...
//    }
//    
//    if(d->transfers[line].state != SDO_RESET)
//    {
//        getWriteResultNetworkDict_error++;
//    }
