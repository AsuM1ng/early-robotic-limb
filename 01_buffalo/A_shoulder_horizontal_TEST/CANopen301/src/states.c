...
#define PRODUCER_HEARTBEAT_TIME     1000    //ms
...
/**************************************************************************************************************************/
    ...
    UNS8  TPDO1TransmissionType = TRANS_EVENT_PROFILE;//TRANS_EVENT_SPECIFIC;	
    UNS32 TPDO1_inhibit_time = 200;         // TPDO抑制时间 unit: 200μs
    UNS32 TPDO1_event_time = 20;           // TPDO触发时间 unit: 1ms
    ...
    UNS32 TPDO1MappingValue1 = 0x60670120;  /*PDO发送肩关节水平节点角度*/
#elif ('B' == JOINTBOARD_TYPE)
    UNS32 TPDO1MappingValue1 = 0x60670220;  /*PDO发送肩关节垂直节点角度*/
#elif ('C' == JOINTBOARD_TYPE)
    UNS32 TPDO1MappingValue1 = 0x60670320;  /*PDO发送上臂旋转节点角度*/
#elif ('D' == JOINTBOARD_TYPE)
    UNS32 TPDO1MappingValue1 = 0x60670420;  /*PDO发送肘关节节点角度*/
#elif ('E' == JOINTBOARD_TYPE)
    UNS32 TPDO1MappingValue1 = 0x60670520;  /*PDO发送前臂旋转节点角度*/
#elif ('F' == JOINTBOARD_TYPE)
    UNS32 TPDO1MappingValue1 = 0x60670620;  /*PDO发送腕关节节点角度*/
#else
    ...
    /*RPDO4 configure */
    UNS32 RPDO4_COBID = 0x183;  /*肩关节水平节点，节点ID = 3*/
    UNS8  RPDO4TransmissionType = TRANS_EVENT_SPECIFIC;
    
    setODentry(d, 0x1403, 0x01, &RPDO4_COBID, &sizeUNS32, OD_ACCESS_RW);
    setODentry(d, 0x1403, 0x02, &RPDO4TransmissionType, &sizeUNS8, OD_ACCESS_RW);
    
    UNS32 RPDO4MappingValue1 = 0x60670120;      /*"肩关节水平节点角度x100"*/
    setODentry(d, 0x1603, 0x01, &RPDO4MappingValue1, &sizeUNS32, OD_ACCESS_RW);
#endif
    ...
    /*RPDO5 configure */
    UNS32 RPDO5_COBID = 0x184;  /*肩关节垂直节点，节点ID = 4*/
    UNS8  RPDO5TransmissionType = TRANS_EVENT_SPECIFIC;
    
    setODentry(d, 0x1404, 0x01, &RPDO5_COBID, &sizeUNS32, OD_ACCESS_RW);
    setODentry(d, 0x1404, 0x02, &RPDO5TransmissionType, &sizeUNS8, OD_ACCESS_RW);
    
    UNS32 RPDO5MappingValue1 = 0x60670220;      /*"肩关节垂直节点角度x100"*/
    setODentry(d, 0x1604, 0x01, &RPDO5MappingValue1, &sizeUNS32, OD_ACCESS_RW);
#endif

#if(SELF_NODE_ID  != 5)
    /********************************************上臂旋转节点角度接收RPDO参数配置********************************************/
    /*RPDO6 configure */
    UNS32 RPDO6_COBID = 0x185;  /*上臂旋转节点，节点ID = 5*/
    UNS8  RPDO6TransmissionType = TRANS_EVENT_SPECIFIC;
    
    setODentry(d, 0x1405, 0x01, &RPDO6_COBID, &sizeUNS32, OD_ACCESS_RW);
    setODentry(d, 0x1405, 0x02, &RPDO6TransmissionType, &sizeUNS8, OD_ACCESS_RW);
    
    UNS32 RPDO6MappingValue1 = 0x60670320;      /*"上臂旋转节点角度x100"*/
    setODentry(d, 0x1605, 0x01, &RPDO6MappingValue1, &sizeUNS32, OD_ACCESS_RW);
#endif

#if(SELF_NODE_ID != 6)
    /********************************************肘关节角度接收RPDO参数配置********************************************/
    /*RPDO7 configure */
    UNS32 RPDO7_COBID = 0x186;  /*肘关节节点，节点ID = 6*/
    UNS8  RPDO7TransmissionType = TRANS_EVENT_SPECIFIC;
    
    setODentry(d, 0x1406, 0x01, &RPDO7_COBID, &sizeUNS32, OD_ACCESS_RW);
    setODentry(d, 0x1406, 0x02, &RPDO7TransmissionType, &sizeUNS8, OD_ACCESS_RW);
    
    UNS32 RPDO7MappingValue1 = 0x60670420;      /*"肘关节节点角度x100"*/
    setODentry(d, 0x1606, 0x01, &RPDO7MappingValue1, &sizeUNS32, OD_ACCESS_RW);
#endif

#if(SELF_NODE_ID != 7)
    /********************************************前臂旋转节点角度接收RPDO参数配置********************************************/
    /*RPDO8 configure */
    UNS32 RPDO8_COBID = 0x187;  /*前臂旋转节点，节点ID = 7*/
    UNS8  RPDO8TransmissionType = TRANS_EVENT_SPECIFIC;
    
    setODentry(d, 0x1407, 0x01, &RPDO8_COBID, &sizeUNS32, OD_ACCESS_RW);
    setODentry(d, 0x1407, 0x02, &RPDO8TransmissionType, &sizeUNS8, OD_ACCESS_RW);
    
    UNS32 RPDO8MappingValue1 = 0x60670520;      /*"前臂旋转节点角度x100"*/
    setODentry(d, 0x1607, 0x01, &RPDO8MappingValue1, &sizeUNS32, OD_ACCESS_RW);
#endif

#if(SELF_NODE_ID != 8)
    /********************************************腕关节节点角度接收RPDO参数配置********************************************/
    /*RPDO9 configure */
    UNS32 RPDO9_COBID = 0x188;  /*腕关节节点，节点ID = 8*/
    UNS8  RPDO9TransmissionType = TRANS_EVENT_SPECIFIC;
    
    setODentry(d, 0x1408, 0x01, &RPDO9_COBID, &sizeUNS32, OD_ACCESS_RW);
    setODentry(d, 0x1408, 0x02, &RPDO9TransmissionType, &sizeUNS8, OD_ACCESS_RW);
    
    UNS32 RPDO9MappingValue1 = 0x60670620;      /*"腕关节节点角度x100"*/
    setODentry(d, 0x1608, 0x01, &RPDO9MappingValue1, &sizeUNS32, OD_ACCESS_RW);
#endif
}
...
//        UNS32 TPDO1_inhibit_time = 200;        // unit: 100μs
//        UNS32 TPDO1_event_time = 5;         // unit: 1ms
//        
//        //设置CSP位置
//        UNS32 TPDO1MappingValue1 = 0x2F120020;      // CSP模式下desired位置,经阻抗控制修改后的值
//        setODentry(d, 0x1800, 0x01, &TPDO1_COBID, &sizeUNS32, OD_ACCESS_RW);
//        setODentry(d, 0x1800, 0x02, &TPDO1TransmissionType, &sizeUNS8, OD_ACCESS_RW);
//        setODentry(d, 0x1800, 0x03, &TPDO1_inhibit_time, &sizeUNS32, OD_ACCESS_RW);				
//        setODentry(d, 0x1800, 0x05, &TPDO1_event_time,   &sizeUNS32, OD_ACCESS_RW);
...
//    }
	...
	/*RPDO Configuration*/
	{
		...
		UNS32 RPDO1MappingValue2 = 0x606C0020;							//读取实际速度
		...
		UNS32 RPDO2MappingValue1 = 0x603F0010;							//读取error code
		UNS32 RPDO2MappingValue2 = 0x60410010;							//读取Statusword
...
//		UNS32 RPDO3MappingValue1 = 0x30D20001;							//读取力矩平均值
//		
